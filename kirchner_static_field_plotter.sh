...
for auxiliary_file in "2a" "2b"; do
  for static_field in 1 2 3; do
    ...
    ./run.sh -genvironment_heatmap --wall-threshold=-1000 -iin/${dir_name}/aux_kirchner_fig${auxiliary_file}.txt_static_field_${static_field}.txt \
              -o${dir_name}/kirchner_fig${auxiliary_file}_static-field${static_field}.png --only-save-fig
  done
done